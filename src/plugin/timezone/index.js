...
export default (o, c, d) => {
  ...
  proto.tz = function (timezone = defaultTimezone, keepLocalTime) {
    ...
    const oldDateOffset = (-Math.round(date.getTimezoneOffset() / 15) * 15)
    const target = date.toLocaleString('en-US', { timeZone: timezone })
    const diff = Math.round((date - new Date(target)) / 1000 / 60)
    const offset = oldDateOffset - diff
    ...
    if (isUTC) { // if utcOffset is 0, turn it to UTC mode
      ...
      if (keepLocalTime) {
        ...
        if (newDateOffset !== oldDateOffset) {
          ins = ins.add(oldDateOffset - newDateOffset, MIN)
        }
      }
    }
